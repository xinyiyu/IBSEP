...
def generate_data(h1sq, h2sq, rg, pi1, Nc, Nt, pcau, delta, r, save_dir, seed, G=None, ieqtl=False):
    ...
    if metric == 'power':
        ...
        # split the region into 0.8+0.2
        cutoff = 0.85
        Omega1 = np.array([[h1sq, np.sqrt(h1sq*h2sq)*rg], [np.sqrt(h1sq*h2sq)*rg, h2sq]]) / (pcau * M)
        Omega2 = np.array([[h2sq, np.sqrt(h2sq*h2sq)*rg], [np.sqrt(h2sq*h2sq)*rg, h2sq]]) / (pcau * M)
        ...
        cau_ids = sorted(cau_ids)
        beta_cau1 = np.random.multivariate_normal(mean=np.zeros(2), cov=Omega1, size=causal_snps_num1)
        beta_cau2 = np.random.multivariate_normal(mean=np.zeros(2), cov=Omega2, size=causal_snps_num-causal_snps_num1)
        beta_cau = np.vstack((beta_cau1,beta_cau2))
        beta1[cau_ids] = beta_cau[:, 0]
        beta2[cau_ids] = beta_cau[:, 1]
        y1 = Xc @ beta1 + np.sqrt(1 - h1sq*cutoff-h2sq*(1-cutoff)) * np.random.randn(Nc)
        y2 = Xc @ beta2 + np.sqrt(1 - h2sq) * np.random.randn(Nc)
...
if __name__ == '__main__':
    ...
    pcaus = [0.005, 0.02, 0.05] # lower pcau
    delta = args.delta
    nrep = 100
    start = 0
